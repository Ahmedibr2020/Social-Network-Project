<div class="usernav">
    <?php
        ...
        $sqlll= "SELECT COUNT(*) AS countt FROM notifications
                 WHERE notifications.usr2_id = {$_SESSION['user_id']} AND notifications.notif_status = 0";
        $queryyy=mysqli_query($conn, $sqlll);
        $rowww = mysqli_fetch_assoc($queryyy);
    ?>
    <ul> <!-- Ensure there are no enter escape characters.-->
        <li><a href="noti.php">Notifications (<?php echo $rowww['countt'] ?>)</a><a href="requests.php">Friend Requests (<?php echo $row['count'] ?>)</a></li><li><a href="profile.php">Profile</a></li><li><a href="friends.php">Friends</a></li><li><a href="home.php">Home</a></li><li><a href="logout.php">Log Out</a></li>
    </ul>
